...
# Add naive HA.
r "applyPersistentVolumeWithGeneratedMetrics eu1 1 6h"
...
r "colordiff -y manifests/grafana-datasources.yaml manifests/grafana-ha-datasources.yaml | less -X"
ro "kubectl --context=eu1 apply -f manifests/grafana-ha-datasources.yaml" "kubectl --context=eu1 apply -f manifests/grafana-ha-datasources.yaml && kubectl --context=eu1 delete po \$(kubectl --context=eu1 get po -l app=grafana -o jsonpath={.items..metadata.name})"
r "kubectl --context=eu1 get po"
ro "open \$(minikube -p eu1 service grafana --url)" "google-chrome --app=\"`minikube -p eu1 service grafana --url`/d/pods_memory/pods-memory?orgId=1\" > /dev/null"
...
r "colordiff -y manifests/grafana-ha-datasources.yaml manifests/grafana-datasources-querier.yaml | less -X"
